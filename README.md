...
- Instances : 실행할 인스턴스 수
- Pack : 열고 싶은 팩
- Columns : 기본 5인데 인스턴스 많이 안쓰면 그냥 냅두면됨
- Language : Korean선택
- Delay : 동작 간 딜레이. 뭔가 렉너무걸린다 하면 이거 수정(기본250인가?쓰면됨)
- Time Zone: 게임 날짜 변경 시간 - 1500으로 설정
- 이름: 계정 이름
- 행당 인스턴스: 한 줄에 표시할 인스턴스 수
- Netease 경로: 기본 설치 경로 사용 시 그대로 두기, 변경했다면 새 경로 입력
- 속도: 새 스피드 모드 사용 시 게임 속도 설정
  - 2x는 구버전과 신버전 모두 사용 가능
  - 신버전 사용 시: platinmods 로고 > 설정 > 환경설정 저장
- 언어: 게임 언어 선택
- 모니터: 실행할 모니터 선택. 문제 방지를 위해 MuMu 인스턴스를 올바른 모니터에서 시작
- 스와이프 속도: 스와이프 지속 시간. 시스템에 따라 조절
- 갓팩: 갓팩 발견 시 일시정지 또는 리소스 절약을 위한 에뮬레이터 종료 선택
- 다음 클릭
